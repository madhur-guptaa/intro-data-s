...
def plotEDF(edf, force_display=True, points_at_jump=True, confidence_band=False, alpha=0.95, color='blue',
...
def classification_report_interval(
    ...
    def precision_recall(y_true,
        ...
        for label in labels:
            ...
            p.append(
                "%.2f : [%.2f,%.2f]" % (precision, np.maximum(precision - delta, 0), np.minimum(precision + delta, 1)))
